...
define([
  ...
  function (angular, app, $, _, kbn, moment, timeSeries, numeral) {
    ...
    var module = angular.module('kibana.panels.histogram2', []);
    app.useModule(module);

    module.controller('histogram2', function($scope, querySrv, dashboard, filterSrv) {
      $scope.panelMeta = {
        modals : [
          {
            description: "Inspect",
      ...
      $scope.get_data = function(data, segment, query_id) {
        ...
        _.each(queries, function(q) {
          ...
          request = request
            ...
            .agg(ejs.FilterAggregation(q.id)
              .filter(filter)
              .agg(aggr));

        });

        // I do not know what it suppose to do
        //if(!$scope.panel.annotate.enable) {
